...
router.get("/getAllReviews", async (req, res) => {
    try {
        const reviews = await Review.find().populate('productId', 'name').populate('userId', 'username');
...
router.get("/getReviewByProductId/:productId", async (req, res) => {
    try {
        ...
        if (!productExists) {
            return res.status(404).json({ message: 'Bu ID ile eşleşen bir ürün bulunamadı.' });
        }

        // Ürün varsa yorumları getiriyorum
...
router.put("/updateReview/:id", async (req, res) => {
    try {
        ...
        if (!updatedReview) return res.status(404).json({ message: 'Yorum bulunamadı.' });

        res.status(200).json({ message: 'Yorum başarıyla güncellendi.', review: updatedReview });
    } catch (error) {
        res.status(500).json({ message: 'Bir hata oluştu.', error: error.message });
    }
});
